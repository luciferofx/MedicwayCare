...
import {
  SortAsc, SortDesc, X, SlidersHorizontal, UserRound,
  Calendar, Clock, Phone, Mail, FileText, CheckCircle2,
...
export default function DoctorListingPage() {
  ...
  const { data, isLoading, isError } = useGetAllDoctorsQuery({
    country, category, city, treatment, hospital,
    priceRange, rating, experience, availability, page, limit,
  });

  const doctors    = data?.data?.data   || [];
  const totalItems = data?.data?.total  || doctors.length;
  ...
  const sortedDoctors = useMemo(() => {
    return [...doctors].sort((a, b) => {
      ...
      switch (sortBy) {
        ...
        case "location":   aVal = a.location?.city || "";  bVal = b.location?.city || "";  break;
  ...
  const [appointmentForm, setAppointmentForm] = useState({
    patientName: "", phone: "", email: "", date: "", time: "", message: "",
  });
      ...
      <div className="absolute -top-12 -right-12 w-72 h-72 bg-white/5 rounded-full pointer-events-none" />
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="inline-flex items-center gap-2 bg-white/15 backdrop-blur-sm border border-white/20 rounded-full px-4 py-1.5 mb-5">
            ...
            <span className="text-sm font-medium text-blue-50 tracking-wide">Medical Professionals</span>
          </div>
          ...
          <p className="text-blue-100 max-w-xl text-base sm:text-lg leading-relaxed">
            Connect with highly qualified specialists offering expert care, compassionate service, and accessible healthcare.
          </p>
        </div>
      </div> 
          ...
          <div className="lg:col-span-3 space-y-5">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white border border-gray-200 rounded-xl px-4 py-3 shadow-sm">
                ...
                <div className="flex items-center gap-2">
                  <select
                    ...
                    className="border border-gray-300 rounded-lg px-3 py-2 text-sm font-medium text-gray-700 bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition"
                  >
            ...
            {isLoading && (
              <div className="flex flex-col justify-center items-center py-32 gap-4">
                ...
                <p className="text-gray-400 text-sm font-medium">Finding doctors for you…</p>
              </div>
            )}
            ...
            {isError && (
              <div className="bg-white border border-red-200 rounded-2xl p-12 text-center shadow-sm">
                ...
                <h3 className="text-xl font-bold text-red-700 mb-2">Unable to Load Doctors</h3>
              ...
              >
                {sortedDoctors.map((doctor) => (
                  <motion.div key={doctor._id} variants={itemVariants}>
                    <DoctorCard doc={doctor} onBook={() => openModal(doctor)} />
                  </motion.div>
                ))}
              </motion.div>
            )}
          ...
          <div className="relative bg-white w-full sm:max-w-lg rounded-t-3xl sm:rounded-2xl shadow-2xl flex flex-col max-h-[95vh] sm:max-h-[90vh] overflow-hidden modal-slide-up">
            ...
            <div className="bg-gradient-to-r from-[#0a2a55] to-[#1e3a8a] px-5 pt-5 pb-5 flex-shrink-0">
              ...
              <div className="flex items-center justify-between">
                <h3 className="text-base font-bold text-white tracking-tight">Book Appointment</h3>
                <button
                  onClick={closeModal}
              ...
              {/* Doctor strip */}
              <div className="mt-4 flex items-center gap-3 bg-white/10 rounded-xl px-3 py-3">
                ...
                <div>
                  <p className="font-bold text-white text-sm leading-tight">Dr. {selectedDoctor.name}</p>
                  <p className="text-teal-100 text-xs">{selectedDoctor.categoryData?.name || "Specialist"}</p>
                </div>
              </div>
            </div>
            ...
            {bookingSuccess ? (
              <div className="flex-1 flex flex-col items-center justify-center text-center px-8 py-12">
                ...
                <h4 className="text-lg font-bold text-gray-900 mb-2">Appointment Confirmed!</h4>
                <p className="text-gray-500 text-sm mb-6 max-w-xs">
                  Your appointment with <span className="font-semibold text-gray-700">Dr. {selectedDoctor.name}</span> is booked. You'll receive a confirmation shortly.
                </p>
                <button
                  onClick={closeModal}
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <select
                          ...
                          onChange={handleFormChange}
                          className={`${inputWithIconCls} appearance-none bg-white`}
                          required
                        >
